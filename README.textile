...
The terrain density texture is 2048x512 (You could go as high as 2048x2048 though). When rendering, this is upscaled by 16x (32x on retina) giving you a fully deformable world that is over 130 screen widths wide. Rendering the world is performed by a shader that renders all of the following in a single pass:
...
* Because the terrain is drawn using a single gigantic quad, the precision of the texture samplers runs out about halfway across the world. This effectively halves the texture resolution. It could be fixed easily enough by splitting the quad into multiple pieces though.
